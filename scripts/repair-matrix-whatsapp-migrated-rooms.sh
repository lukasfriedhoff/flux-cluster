...
admin_user_id="${ADMIN_USER_ID:-@lukasf:h4xx.io}"
bridge_user_id="${BRIDGE_USER_ID:-@whatsappbot:h4xx.io}"
...
usage() {
  cat <<'EOF'
Usage:
  repair-matrix-whatsapp-migrated-rooms.sh inventory ROOM_FILE
  repair-matrix-whatsapp-migrated-rooms.sh repair ROOM_FILE
...
Required environment:
  ACCESS_TOKEN       Synapse admin access token
    or
  ACCESS_TOKEN_FILE  File containing the Synapse admin access token

Optional environment:
  SYNAPSE_URL             Default: http://127.0.0.1:8008
  ADMIN_USER_ID           Default: @lukasf:h4xx.io
  BRIDGE_USER_ID          Default: @whatsappbot:h4xx.io
  REQUIRED_POWER          Default: 100
  MAX_ATTEMPTS            Default: 8
  REQUEST_DELAY_SECONDS   Default: 1

The repair command only calls Synapse's supported make_room_admin API when the
configured ADMIN_USER_ID already has at least REQUIRED_POWER in the room. It
does not edit Synapse event or state tables.
EOF
}

die() {
  echo "repair-matrix-whatsapp-migrated-rooms: $*" >&2
  exit 1
}
...
admin_user_id, bridge_user_id = sys.argv[1:]
response = json.load(sys.stdin)
default_power = response.get("users_default", 0)
users = response.get("users") or {}
print(
    f"{users.get(admin_user_id, default_power)}\t"
    f"{users.get(bridge_user_id, default_power)}"
)
' "$admin_user_id" "$bridge_user_id" <"$response_file"
}
...
read_power_levels() {
  ...
  if ! get_power_levels "$room_id"; then
  ...
  if ! powers="$(parse_power_levels)"; then
    printf '%s\t-\t-\tinvalid-power-levels\t-\n' "$room_id"
    return 1
  fi

  IFS=$'\t' read -r admin_power bridge_power <<<"$powers"
}
...
inventory_room() {
  ...
  if ! read_power_levels "$room_id"; then
    return 1
  fi

  if ((bridge_power >= required_power)); then
    status="repaired"
  elif ((admin_power >= required_power)); then
    status="repairable"
  else
    status="insufficient-local-admin"
  fi

  printf '%s\t%s\t%s\t%s\t-\n' \
    "$room_id" "$admin_power" "$bridge_power" "$status"
}

repair_room() {
  local room_id="$1"
  local attempt delay_ms delay_seconds error

  if ! read_power_levels "$room_id"; then
    return 1
  fi
  if ((bridge_power >= required_power)); then
    printf '%s\t%s\t%s\talready-repaired\t-\n' \
      "$room_id" "$admin_power" "$bridge_power"
    return 0
  fi
  if ((admin_power < required_power)); then
    printf '%s\t%s\t%s\tskipped-insufficient-local-admin\t-\n' \
      "$room_id" "$admin_power" "$bridge_power"
    return 1
  fi

  for ((attempt = 1; attempt <= max_attempts; attempt++)); do
    if ! make_bridge_admin "$room_id"; then
      printf '%s\t%s\t%s\trepair-request-failed\t%s\n' \
        "$room_id" "$admin_power" "$bridge_power" "$http_status"
      return 1
    fi
    if [[ "$http_status" =~ ^2 ]]; then
      break
    ...
    if [[ "$http_status" == "429" ]]; then
      ...
      echo "Rate limited while repairing $room_id; retrying in ${delay_seconds}s." >&2
      sleep "$delay_seconds"
      continue
    fi

    error="$(json_error)"
    printf '%s\t%s\t%s\trepair-error-%s\t%s\n' \
      "$room_id" "$admin_power" "$bridge_power" "$http_status" "$error"
    return 1
  done

  if [[ ! "$http_status" =~ ^2 ]]; then
    printf '%s\t%s\t%s\trepair-retries-exhausted\t%s\n' \
      "$room_id" "$admin_power" "$bridge_power" "$http_status"
    return 1
  fi
  ...
  if ((bridge_power < required_power)); then
    printf '%s\t%s\t%s\tverification-failed\t-\n' \
      "$room_id" "$admin_power" "$bridge_power"
    return 1
  fi

  printf '%s\t%s\t%s\trepaired\t-\n' \
    "$room_id" "$admin_power" "$bridge_power"
}
...
process_rooms() {
  ...
  printf 'room_id\tadmin_power\tbridge_power\tstatus\terror\n'
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    [[ -n "$line" && "$line" != \#* && "$line" != room_id$'\t'* ]] || continue
    room_id="${line%%$'\t'*}"
    [[ "$room_id" == '!'*:* ]] || {
      echo "Skipping invalid room ID: $room_id" >&2
      ((failed += 1))
      continue
    }
